...
const APP_STATUSES = ['draft','submitted','awarded','declined'];
const appStatusStyle = (s) => {
  if (s === 'submitted') return { background:'#E7EEF7', color:'#2A4A78' };
  if (s === 'awarded') return { background: SAGE, color: FOREST };
  if (s === 'declined') return { background:'#FBEAEA', color:'#9A2B2B' };
  return { background:'#F0EDE6', color:'#7A6F58' }; // draft
};

// ============ GRANTS TAB ============
export default function GrantsTab({ user, orgName }) {
  const [view, setView] = useState('grants'); // 'grants' | 'apps'
  ...
  const [profile, setProfile] = useState({
    orgName: orgName || 'First Fruits Christian Center', location:'Houston, TX',
    orgType:'501(c)(3) church / faith-based nonprofit', focusAreas:'', need:'',
  });
  ...
  const loadGrants = async () => {
    ...
    try {
      ...
      if (error) setError(error.message); else { setGrants(data || []); setError(null); }
  ...
  useEffect(() => { loadGrants(); loadApps(); }, []);

  const grantById = useMemo(() => {
    const m = {}; grants.forEach(g => { m[g.id] = g; }); return m;
  }, [grants]);
  ...
  const visible = useMemo(() => {
    let rows = grants.filter(g => {
      ...
      const ms = !q || [g.name, g.funder, g.focus_area, g.eligibility].filter(Boolean).some(v => v.toLowerCase().includes(q));
      const mf = focus === 'all' || g.focus_area === focus;
      const mst = status === 'all' || g.status === status;
      return ms && mf && mst;
    });
    rows = [...rows].sort((a,b) => {
  ...
  const insertGrant = async (row) => {
    ...
    try {
      ...
      setGrants(p => [...p, data]); return data;
    } catch(e) { alert('Save failed: ' + (e.message || 'unknown')); return null; }
  };

  const handleAdd = async () => {
    if (!f.name.trim()) { alert('Grant name is required.'); return; }
    await insertGrant({ name: f.name.trim(), funder: f.funder.trim() || null, amount: f.amount ? parseFloat(f.amount) : null, deadline: f.deadline || null, focus_area: f.focus_area.trim() || null, eligibility: f.eligibility.trim() || null, link: f.link.trim() || null, status: f.status });
    setShowAdd(false);
  ...
  const acceptSuggestion = async (g, idx) => {
    await insertGrant({ name: g.name, funder: g.funder || null, amount: g.amount ?? null, deadline: g.deadline || null, focus_area: g.focus_area || null, eligibility: g.eligibility || null, link: g.link || null, status:'open' });
  ...
  const openDraft = async (g) => {
    ...
    setDp(d => ({ ...d, orgName: orgName || DEFAULT_ORG, amount: g.amount ? String(g.amount) : d.amount }));
    try {
      const sb = await getSupabase();
  ...
  const generateDraft = async () => {
    ...
    try {
      const resp = await fetch('/api/draft-application', { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ grant: draftGrant, ...dp }) });
      const data = await resp.json();
  ...
  const saveApplication = async () => {
    ...
    try {
      ...
      if (existingAppId) {
        ...
        const { data, error } = await sb.from('applications').insert({ grant_id: draftGrant.id, org_name: dp.orgName, draft_text: draftText, submission_status:'draft' }).select().single();
        if (error) { setSaveMsg('Save error: ' + error.message); return; }
        if (data) setExistingAppId(data.id);
      }
      setSaveMsg('Saved ✓'); loadApps();
  ...
  const updateApp = async (id, patch) => {
    ...
    try { const sb = await getSupabase(); await sb.from('applications').update(patch).eq('id', id); } catch(e) {}
  };

  const deleteApp = async (id) => {
    if (!confirm('Delete this application? The grant stays in your directory.')) return;
    setApps(prev => prev.filter(a => a.id !== id));
    try { const sb = await getSupabase(); await sb.from('applications').delete().eq('id', id); } catch(e) {}
  };

  const setAppStatus = (a, newStatus) => {
    const patch = { submission_status: newStatus };
    if (newStatus === 'submitted' && !a.submitted_date) patch.submitted_date = new Date().toISOString().slice(0,10);
    updateApp(a.id, patch);
  };

  const openDraftFromApp = (a) => {
    const g = grantById[a.grant_id];
    if (g) openDraft(g);
    else alert('The grant for this application was removed from the directory.');
  };

  const stats = useMemo(() => {
    const s = { draft:0, submitted:0, awarded:0, declined:0, awardedTotal:0 };
    apps.forEach(a => {
      const st = a.submission_status || 'draft';
      if (s[st] != null) s[st]++;
      if (st === 'awarded') { const g = grantById[a.grant_id]; s.awardedTotal += (g && g.amount ? Number(g.amount) : 0); }
    });
    return s;
  }, [apps, grantById]);
  ...
  const tabBtn = (active) => ({ padding:'8px 18px', border:`1px solid ${active ? NAVY : BORDER}`, background: active ? NAVY : '#fff', color: active ? '#fff' : NAVY, borderRadius:8, cursor:'pointer', fontWeight:700, fontSize:'0.9rem' });
    ...
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1rem', flexWrap:'wrap', gap:8 }}>
        <div>
          ...
          <p style={{ color: TXT_LIGHT, fontSize:'0.9rem', marginTop:4 }}>Find grants, draft applications, and track every one.</p>
        </div>
        {view === 'grants' && (
      ...
      <div style={{ display:'flex', gap:8, marginBottom:'1.5rem' }}>
        <button style={tabBtn(view==='grants')} onClick={()=>setView('grants')}>Grants{grants.length ? ` (${grants.length})` : ''}</button>
        <button style={tabBtn(view==='apps')} onClick={()=>setView('apps')}>Applications{apps.length ? ` (${apps.length})` : ''}</button>
      </div>
      ...
      <div>
        ...
        {error && <div className="card card-p" style={{ color:'#B53232', borderLeft:'4px solid #B53232' }}>Could not load grants: {error}.</div>}
      ...
      <div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(120px, 1fr))', gap:'0.75rem', marginBottom:'1.5rem' }}>
          {[['Draft', stats.draft, '#7A6F58'], ['Submitted', stats.submitted, '#2A4A78'], ['Awarded', stats.awarded, FOREST], ['Declined', stats.declined, '#9A2B2B']].map(([lab, n, col]) => (
            <div key={lab} className="card card-p" style={{ textAlign:'center' }}>
              <div style={{ fontSize:'1.8rem', fontWeight:800, color: col }}>{n}</div>
              <div style={{ fontSize:'0.78rem', color: TXT_LIGHT, textTransform:'uppercase', fontWeight:700, letterSpacing:'0.04em' }}>{lab}</div>
            </div>
          ))}
          <div className="card card-p" style={{ textAlign:'center', borderTop:`3px solid ${GOLD}` }}>
            <div style={{ fontSize:'1.8rem', fontWeight:800, color: NAVY }}>{fmtMoney(stats.awardedTotal)}</div>
            <div style={{ fontSize:'0.78rem', color: TXT_LIGHT, textTransform:'uppercase', fontWeight:700, letterSpacing:'0.04em' }}>Awarded $</div>
          </div>
        </div>

        {appsLoading && <div className="card card-p" style={{ textAlign:'center', color: TXT_LIGHT }}>Loading applications...</div>}

        {!appsLoading && apps.length === 0 && (
          <div className="card card-p" style={{ textAlign:'center', padding:'3rem' }}>
            <div style={{ fontSize:'2.5rem', marginBottom:8 }}>✍️</div>
            <h3 style={{ marginBottom:6 }}>No applications yet</h3>
            <p style={{ color: TXT_LIGHT, marginBottom:'1.5rem' }}>Draft an application from a grant, save it, and it shows up here to track.</p>
            <button className="btn btn-navy" onClick={()=>setView('grants')}>Go to grants</button>
          </div>
        )}
        ...
        <div style={{ display:'grid', gap:'0.85rem' }}>
          {apps.map(a => {
            const g = grantById[a.grant_id];
            return (
              <div key={a.id} className="card card-p" style={{ borderLeft:`4px solid ${appStatusStyle(a.submission_status).color}` }}>
                <div style={{ display:'flex', justifyContent:'space-between', gap:12, alignItems:'flex-start', flexWrap:'wrap' }}>
                  <div style={{ flex:'1 1 240px' }}>
                    <div style={{ fontWeight:700, color: NAVY, fontSize:'1.05rem' }}>{g ? g.name : '(grant removed)'}</div>
                    <div style={{ fontSize:'0.85rem', color: TXT_LIGHT }}>{a.org_name || DEFAULT_ORG}{g && g.funder ? ' · ' + g.funder : ''}</div>
                    <div style={{ fontSize:'0.82rem', color: TXT_LIGHT, marginTop:4 }}>Award: {fmtMoney(g && g.amount)}{g && g.deadline ? ' · Deadline ' + fmtDate(g.deadline) : ''}</div>
                  </div>
                  <span style={{ ...appStatusStyle(a.submission_status), fontSize:'0.72rem', fontWeight:700, textTransform:'uppercase', padding:'4px 11px', borderRadius:999, height:'fit-content' }}>{a.submission_status || 'draft'}</span>
                </div>

                <div style={{ display:'flex', gap:14, flexWrap:'wrap', marginTop:14, alignItems:'flex-end' }}>
                  <div>
                    <label style={{ fontSize:'0.7rem', fontWeight:700, color: TXT_LIGHT, textTransform:'uppercase', display:'block', marginBottom:4 }}>Status</label>
                    <select value={a.submission_status || 'draft'} onChange={e=>setAppStatus(a, e.target.value)}>
                      {APP_STATUSES.map(s => <option key={s} value={s}>{s.charAt(0).toUpperCase()+s.slice(1)}</option>)}
                    </select>
                  </div>
                  <div>
                    <label style={{ fontSize:'0.7rem', fontWeight:700, color: TXT_LIGHT, textTransform:'uppercase', display:'block', marginBottom:4 }}>Submitted</label>
                    <input type="date" value={a.submitted_date || ''} onChange={e=>updateApp(a.id, { submitted_date: e.target.value || null })} />
                  </div>
                  <div style={{ flex:'1 1 200px' }}>
                    <label style={{ fontSize:'0.7rem', fontWeight:700, color: TXT_LIGHT, textTransform:'uppercase', display:'block', marginBottom:4 }}>Decision / notes</label>
                    <input style={{ width:'100%' }} defaultValue={a.decision || ''} placeholder="e.g., awarded $10k; reapply next cycle" onBlur={e=>{ if ((e.target.value||'') !== (a.decision||'')) updateApp(a.id, { decision: e.target.value || null }); }} />
                  </div>
                </div>

                <div style={{ display:'flex', gap:8, marginTop:14, alignItems:'center', flexWrap:'wrap' }}>
                  <button className="btn btn-outline" style={{ fontSize:'0.82rem', padding:'7px 12px' }} onClick={()=>openDraftFromApp(a)} disabled={!g}>View / edit draft</button>
                  <button onClick={()=>deleteApp(a.id)} style={{ background:'none', border:'none', cursor:'pointer', fontSize:15, color: TXT_LIGHT, marginLeft:'auto' }}>🗑</button>
                </div>
              </div>
            );
          })}
